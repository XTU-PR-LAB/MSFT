...
def eval(args):
    ...
    psnr_value = 10 ** (-np.mean(psnr_scores)/ 10)
    
    # 计算sqrt(1 - SSIM)
    ssim_value = np.sqrt(1 - np.mean(ssim_scores))
    
    # 计算几何平均值
    avg_mean = (psnr_value * ssim_value * np.mean(lpips_scores)) ** (1/3)
    # 检查结果文件是否存在
    ...
    results["scenes"].extend(args.eval_scenes)
    results["psnr"].append(np.mean(psnr_scores))
    results["lpips"].append(np.mean(lpips_scores))
    results["ssim"].append(np.mean(ssim_scores))
    results["avg"].append(avg_mean)
    # 将结果写入 JSON 文件
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_val", type=bool, default=True, help="run on val set")
