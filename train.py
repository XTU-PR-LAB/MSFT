...
def train(args):
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        for train_data in train_loader:
            ...
            rgb_list = ret["outputs_coarse"]["rgb_list"]
            weights = ret["outputs_coarse"]["weights"]
            depth = ret["outputs_coarse"]["depth"]
            losses = []
            ray_batch["rgb"]=ray_batch["rgb"].to(device)
            for rgb in rgb_list:
                ret1 = {"outputs_coarse": {"rgb": rgb, "weights": weights, "depth": depth}}
                loss, scalars_to_log = criterion(ret1["outputs_coarse"], ray_batch, scalars_to_log)
                losses.append(loss)
            total_loss = torch.sum(torch.stack(losses))
            '''a=1
            b=1
            c=1
            total_loss=a*losses[0]+b*losses[1]+c*losses[2]'''
            total_loss.backward()
            ...
            if args.local_rank == 0:
                if global_step % args.i_print == 0 or global_step < 10:
                    ...
                    mse_error = img2mse(ret1["outputs_coarse"]["rgb"], ray_batch["rgb"]).item()
...
def log_view(
    ...
    args.psnr_values.append(psnr_curr_img)
    print("PSNR values:", args.psnr_values)
