...
class Projector:
    ...
    def compute(self, args, xyz, query_camera, train_imgs, train_cameras, featmaps):
        """
        ...
        rgb_feat=[]
        start_index = max(0, len(featmaps) - args.n_scales -1)
        for i_scale in range(start_index,len(featmaps)-1): 
            s=2**(len(featmaps)-2-i_scale)#2**(2-i_scale)
            output = F.interpolate(train_imgs, scale_factor=1/s, mode='bilinear', align_corners=False)
            rgbs_sampled = F.grid_sample(output, normalized_pixel_locations, align_corners=True)
            rgb_sampled = rgbs_sampled.permute(2, 3, 0, 1)  # [n_rays, n_samples, n_views, 3]  
            featmap = featmaps[i_scale]
            feat_sampled = F.grid_sample(featmap, normalized_pixel_locations, align_corners=True)
            ...
            rgb_feat.append(rgb_feat_sampled)
