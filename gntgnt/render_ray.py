...
def render_rays(
    ...
    ray_batch["src_rgbs"]=ray_batch["src_rgbs"][:, 1:11, :, :, :]
    # TODO: include pixel mask in ray transformer
    # pixel_mask = (
    #     mask[..., 0].sum(dim=2) > 1
    # )  # [N_rays, N_samples], should at least have 2 observations

    rgb_list = model.net_coarse(args, rgb_feat, ray_diff, mask, pts, ray_d)
    if ret_alpha:
        ...
        weights = None
        depth_map = None
    ret["outputs_coarse"] = {"rgb_list": rgb_list, "weights": weights, "depth": depth_map}
    ...
    if N_importance > 0:
        ...
        # )  # [N_rays, N_samples]. should at least have 2 observations
        if single_net:
            rgb = model.net_coarse(args, rgb_feat_sampled, ray_diff, mask, pts, ray_d)
        else:
            rgb = model.net_fine(args, rgb_feat_sampled, ray_diff, mask, pts, ray_d)
        rgb, weights = rgb[:, 0:3], rgb[:, 3:]
        depth_map = torch.sum(weights * z_vals, dim=-1)
